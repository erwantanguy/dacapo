        ...
        $artistes = get_field('artiste');
         $menuArtistes = '';
                foreach ($artistes as $artiste){
                    //print_r($artiste);
                    $link = get_post_permalink($artiste->ID);
                    $name = $artiste->post_title;
                    $menuArtistes .= '<li><a href="'.$link.'">'.$name.'</a></li>';
                    //echo 'test';
                    //print_r ($menuArtistes);
                }
                //echo $menuArtistes;
        }
	...
	foreach ( $elements as $element ) {
		...
		if ( 'featured_image' == $element
			...
			get_template_part( 'partials/single/media/blog-single', $format );
                        if($menuArtistes){
                            echo '<dl><dt>Artistes associés à l\'article</dt><dd><ul>'.$menuArtistes.'</ul></dd></dl>';
                        }
